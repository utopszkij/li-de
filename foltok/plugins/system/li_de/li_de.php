...
//if ($browserName == 'Internet Explorer') $browserVersion = 1 + $browserVersion; // ?? valamiért itt rosszat mutat
$browserWarning = '';
if ((($browserName == 'Firefox') & ($browserVersion < 30)) |
    ...
    (($browserName == 'Opera') & ($browserVersion < 30))) {
		$browserWarning = 'Az Ön böngészõ programja elavult, nem támogatott.\n\n'.
		                  'Kérjük frissítse a böngészõjét!\n\n'.
						  'Javasolt böngészõ: Firefox 43, Chrome 47 vagy újabb verzió\n\n'.
						  'Jelenlegi böngészõ:'.$browserName.' '.$browserVersion;
} 
...
class plgSystemLi_de extends JPlugin
{
	...
	/**
	  * az aktuális usernél beállítja a "gr" grupba a dinamikus tagságot ha $value="tru" akkor tag, ha "fasle" akkor nem tag
	  * @param Juser aktuális user
	...
	/**
	  * a task legelején fut le.
	  * 1. spec SEO URL kezelés
	  * 2. dinamikus userGroup tagság beállítás
	  */
	public function onAfterInitialise() {
		//if (JRequest::getVar('limit')=='')
		JRequest::setVar('limit',20);
		
		// dinamikus usergroup kezelés
		$user = JFactory::getUser();
		$db = JFactory::getDBO();
		if ($user->id > 0) {
			// [TG] (témakör gazda), [TT] (témakör tag) és [VG] (vita gazda) usergroupok meghatározása
			$grTT = 0;
			$grTG = 0;
			$grVG = 0;
			...
			if (($grTG > 0) & ($grVG > 0)) {
				...
				// aktuális témakör és szavazás megállapítása
				$temakor = 0;
				$szavazas = 0;
				...
				if (JRequest::getVar('option')=='com_content') {
					// cikk lista vagy cikk megjelenités
				...
				if (JRequest::getVar('option')=='com_jdownloads') {
					// file letöltés listta vagy file oldal 
					if (JRequest::getVar('view')=='category') {
				...
				// ha user tag az adott témakörben akkor [TT] usergroupba felvenni ha nem onnan törölni
				if ($grTT > 0) {
				...
				// ha user tag-admin az aktuális témakörben akkor [TG] usergroupba felvesszük, ha nem onnan töröljük]
				...
				// ha user vita indító az aktuális vitában akkor [VG] usergroupba felvesszük, ha nem onnan töröljük]
		...
		} // user be van jelentkezve (dinamikus usergroup kezelés)
	}
}
